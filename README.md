...
- 自动旋转、边缘线、网格显示
- 零件列表（悬停高亮、点击聚焦）
- **重播动画** — 重新播放零件飞入组装动画，跟随当前视图模式材质
- 导出 GLTF 格式
- 入场飞入动画
- 大模型自适应相机（自动调整裁剪面和缩放范围）

### 2D 图纸查看（DXF / DWG）
- GPU 加粗线渲染（Three.js Line2）
...
### 通用功能
- 拖放或点击上传
- 4 阶段进度条（读取 → 引擎加载 → 解析 → 构建）
- 鼠标悬停显示零件/实体详细参数
- 左键旋转、右键/Shift+左键平移、滚轮缩放
- **方向键平移**（↑↓←→）
- 触屏单指旋转、双指缩放平移
- 浅色技术图纸风格界面
...
- **3D 文件**：首次加载 STEP 时自动下载 OpenCascade WASM（约 15MB），之后浏览器缓存
- **DWG 文件**：首次加载时自动下载 LibreDwg WASM（约 9.5MB）
